...
#define MAX_RESOLVED_ADDRS 8UL
    ...
    for( ulong j=0UL; j<peers_cnt; j++ ) {
      ...
      if( FD_UNLIKELY( 0==num_resolved ) ) {
        ...
        if( FD_UNLIKELY( (ulong)num_resolved+resolved_peers_cnt>=FD_TOPO_SNAPRD_HTTP_PEERS_MAX ) ) {
          FD_LOG_WARNING(( "ignoring peer `%s` in [[snapshots.sources.http.peers]] because reached maximum supported peers %lu", config->firedancer.snapshots.sources.http.peers[ j ].url, FD_TOPO_SNAPRD_HTTP_PEERS_MAX ));
          continue;
        };
        ...
        for( ulong i=1UL; i<(ulong)num_resolved; i++ ) {
          tile->snaprd.http.peers[ resolved_peers_cnt+i ].is_https = tile->snaprd.http.peers[ resolved_peers_cnt ].is_https;
          fd_memcpy( tile->snaprd.http.peers[ resolved_peers_cnt+i ].hostname,
                     tile->snaprd.http.peers[ resolved_peers_cnt ].hostname,
                     sizeof(tile->snaprd.http.peers[ resolved_peers_cnt ].hostname) );
        }
