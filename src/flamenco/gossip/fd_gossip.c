...
ci_change( void *              _ctx,
  ...
  switch( change_type ) {
    ...
    case FD_CRDS_CONTACT_INFO_CHANGE_IDENTITY_CHANGED:
      {
        FD_TEST( stem==NULL ); /* We don't have a stem context here */
        fd_active_set_push_state_t states_to_flush[ FD_ACTIVE_SET_STAKE_BUCKETS ];
        ulong state_cnt = fd_active_set_peer_remove( ctx->active_set, crds_pool_idx, states_to_flush );
        for( ulong i=0UL; i<state_cnt; i++ ) {
          fd_gossip_txbuild_init( states_to_flush[i].txbuild, FD_GOSSIP_MESSAGE_PUSH );
        }
      }
      break;
    default: FD_LOG_ERR(( "Unknown change type %d", change_type )); return;
...
rx_pull_request( fd_gossip_t *                         gossip,
  ...
  fd_gossip_txbuild_stamp_pubkey( pull_resp, gossip->identity_pubkey );
