    ...
    case IN_KIND_REPLAY_OUT: {
      ...
      if( FD_UNLIKELY( sig==REPLAY_SIG_SLOT_COMPLETED ) ) {
        ...
        for( fd_vote_states_iter_t * iter = fd_vote_states_iter_init( iter_, vote_states );
          ...
          if( FD_UNLIKELY( vote_count==FD_RUNTIME_MAX_VOTE_ACCOUNTS ) ) {
        ...
        /* we need to re-fetch parent bank since it's possible that the
           memory was released after acquiring the pointer it but before
           increasing the refcnt */
        fd_bank_t * _parent_bank = fd_banks_get_parent( ctx->banks, bank );
        if( FD_LIKELY( _parent_bank ) ) FD_ATOMIC_FETCH_AND_ADD( &_parent_bank->refcnt, 1UL );
        fd_bank_t * parent_bank = fd_banks_get_parent( ctx->banks, bank );
        if( FD_UNLIKELY( _parent_bank && parent_bank!=_parent_bank ) ) FD_ATOMIC_FETCH_AND_SUB( &_parent_bank->refcnt, 1UL );

        fd_cost_tracker_t const * cost_tracker = fd_bank_cost_tracker_locking_query( bank );
        ulong max_compute_units = !!cost_tracker ? cost_tracker->block_cost_limit : ULONG_MAX;
        ...
        slot_completed.total_txn_cnt          = (uint)(fd_bank_txn_count_get( bank )                 - ( !!parent_bank ? fd_bank_txn_count_get( parent_bank )                : 0UL ));
        slot_completed.vote_txn_cnt           = slot_completed.total_txn_cnt - (uint)(fd_bank_nonvote_txn_count_get( bank )         - ( !!parent_bank ? fd_bank_nonvote_txn_count_get( parent_bank )        : 0UL ));
        slot_completed.failed_txn_cnt         = (uint)(fd_bank_failed_txn_count_get( bank )          - ( !!parent_bank ? fd_bank_failed_txn_count_get( parent_bank )         : 0UL ));
        slot_completed.nonvote_failed_txn_cnt = (uint)(fd_bank_nonvote_failed_txn_count_get( bank )  - ( !!parent_bank ? fd_bank_nonvote_failed_txn_count_get( parent_bank ) : 0UL ));
        ...
        if( FD_LIKELY( parent_bank ) ) FD_ATOMIC_FETCH_AND_SUB( &parent_bank->refcnt, 1UL );
